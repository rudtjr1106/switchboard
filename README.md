...
| **자유 key-value** | 공지 말고 앱이 읽어가는 값(불리언·숫자·글·선택지)을 직접 만들고 고칩니다. 타입·범위·글자 수를 스키마가 검사하고, 안드로이드 쪽에는 기본값이 박힌 `RemoteValues` 클래스로 만들어 줘서 문자열 키 오타가 컴파일 단계에서 걸립니다 |
...
| **값 읽기** | 스키마에서 `RemoteValues` 클래스를 만들어 `values.showEvent` 처럼 씁니다. 오타는 컴파일 에러 | `getBoolean("showEvent")` 처럼 문자열 키. 오타는 실행할 때 드러납니다 |
